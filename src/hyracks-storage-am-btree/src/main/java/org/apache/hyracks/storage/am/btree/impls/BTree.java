...
public class BTree extends AbstractTreeIndex {
    ...
    private boolean insertLeafClean(ITupleReference tuple, int targetTupleIndex, int pageId, BTreeOpContext ctx, UpdateMemo<Integer> memo)
            throws Exception {
        boolean restartOp = false;
        FrameOpSpaceStatus spaceStatus = ctx.getLeafFrame().hasSpaceInsert(tuple);
        ...
        switch (spaceStatus) {
            ...
            case SUFFICIENT_CONTIGUOUS_SPACE: {
                ctx.getModificationCallback().found(null, tuple);
                ctx.getLeafFrame().insert(tuple, targetTupleIndex);
                if(ctx.getLeafFrame().getPage().needsClean()) {
//                	LOGGER.error("Try Cleaning1");
                	((BTreeNSMLeafFrame)ctx.getLeafFrame()).cleanFrame(memo);
                }
                ctx.getSplitKey().reset();
                break;
            }
    ...
    public int vacuumClean(int pageId, IBTreeLeafFrame leafFrame, IBTreeInteriorFrame interiorFrame,
    		UpdateMemo<Integer> memo) throws Exception {
        
        return cleanNextPage(pageId, interiorFrame, leafFrame, memo);
    }
    
    public int cleanNextPage(int pageId, IBTreeInteriorFrame interiorFrame, IBTreeLeafFrame leafFrame, 
    		UpdateMemo<Integer> memo) throws Exception{
    	...
    	if(node != null) {
	    	...
	    	try {
	    		...
	    		if (interiorFrame.isLeaf()) {
	    			...
	    			rightPageId = leafFrame.getNextLeaf();
	//        		clean
	//    			ISerializerDeserializer[] fieldSerdes =
	//                    { IntegerSerializerDeserializer.INSTANCE, IntegerSerializerDeserializer.INSTANCE,
	//                            IntegerSerializerDeserializer.INSTANCE, IntegerSerializerDeserializer.INSTANCE,
	//                            IntegerSerializerDeserializer.INSTANCE, IntegerSerializerDeserializer.INSTANCE };
	//    			LOGGER.error(TreeIndexUtils.printFrameTuples(leafFrame, fieldSerdes));
	        		((BTreeNSMLeafFrame)leafFrame).cleanFrame(memo);
	//        		LOGGER.error(TreeIndexUtils.printFrameTuples(leafFrame, fieldSerdes));
	    		...
	    		if(rightPageId == -1){
	    			MultiComparator cmp = MultiComparator.create(cmpFactories);
	    			rightPageId = cleanFirstPage(rootPage, interiorFrame, leafFrame, cmp, memo);
	//    			LOGGER.error("FoundFirst");
	    		}
	    		
	//    		LOGGER.error("CleanedPageId: " + rightPageId);
	...
	//                for (int i = 0; i < children.size(); i++) {
	//                	nextPage(children.get(i), i == children.size() - 1, leafFrame, interiorFrame, treeHeight,
    ...
    public int cleanFirstPage(int pageId, IBTreeInteriorFrame interiorFrame, IBTreeLeafFrame leafFrame, 
...
//		LOGGER.error(TreeIndexUtils.printFrameTuples(leafFrame, fieldSerdes));
		...
		returnId = cleanFirstPage(((BTreeNSMInteriorFrame) (interiorFrame)).getLeftmostChildPageId(),
    ...
    private void performOpClean(int pageId, ICachedPage parent, boolean parentIsReadLatched, BTreeOpContext ctx, UpdateMemo<Integer> memo)
            throws HyracksDataException {
        ICachedPage node = bufferCache.pin(BufferedFileHandle.getDiskPageId(getFileId(), pageId), false);
        ctx.getInteriorFrame().setPage(node);
        // this check performs an unprotected read in the page
        ...
        try {
            ...
            if (!isLeaf || smFlag) {
                if (!smFlag) {
                    ...
                    while (repeatOp && ctx.getOpRestarts() < MAX_RESTARTS) {
                        int childPageId = ctx.getInteriorFrame().getChildPageId(ctx.getPred());
                        performOpClean(childPageId, node, isReadLatched, ctx, memo);
                        node = null;
                ...
                switch (ctx.getOperation()) {
                    case INSERT: {
                        int targetTupleIndex = ctx.getLeafFrame().findInsertTupleIndex(ctx.getPred().getLowKey());
                        restartOp = insertLeafClean(ctx.getPred().getLowKey(), targetTupleIndex, pageId, ctx, memo);
                        break;
                    }
                    case UPSERT: {
                        int targetTupleIndex = ctx.getLeafFrame().findUpsertTupleIndex(ctx.getPred().getLowKey());
    ...
    public class BTreeAccessor implements ITreeIndexAccessor {
        ...
        public void secondaryInsert(ITupleReference tuple, ITupleAcceptor acceptor)
                throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
//            ctx.setAcceptor(acceptor);
        ...
        public void secondaryInsertVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo, ITupleAcceptor acceptor)
                throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            insert(tuple, ctx);
        ...
        public void secondaryInsertClean(ITupleReference tuple, UpdateMemo<Integer> memo, ITupleAcceptor acceptor)
                throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
//            ctx.setAcceptor(acceptor);
        ...
        public void secondaryInsertCleanVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo, ITupleAcceptor acceptor)
                throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
//            ctx.setAcceptor(acceptor);
            insertClean(tuple, ctx, memo);
        ...
        public void vacuumClean(UpdateMemo<Integer> memo) {
        	ctx.reset();
        	try {
        		this.cleanedPage = btree.vacuumClean(this.cleanedPage, ctx.getLeafFrame(), ctx.getInteriorFrame(), memo);
        	}
        	catch(Exception e) {
        		e.printStackTrace();
        	}
        ...
        private void insertClean(ITupleReference tuple, BTreeOpContext ctx, UpdateMemo<Integer> memo) throws HyracksDataException {
            ctx.getModificationCallback().before(tuple);
            insertUpdateOrDeleteClean(tuple, ctx, memo);
        }
...
//            LOGGER.warn(rec);
        ...
        private void insertUpdateOrDeleteClean(ITupleReference tuple, BTreeOpContext ctx, UpdateMemo<Integer> memo) throws HyracksDataException {
            ctx.reset();
            ctx.getPred().setLowKeyComparator(ctx.getCmp());
            ctx.getPred().setHighKeyComparator(ctx.getCmp());
            ctx.getPred().setLowKey(tuple, true);
...
//            LOGGER.warn(rec);
            ...
            while (repeatOp && ctx.getOpRestarts() < MAX_RESTARTS) {
                ctx.setSmoCount(smoCounter.get());
                performOpClean(rootPage, null, true, ctx, memo);
                // Do we need to restart from the (possibly new) root?
                if (!ctx.getPageLsns().isEmpty()) {
                    if (ctx.getPageLsns().getLast() == FULL_RESTART_OP) {
                        ctx.getPageLsns().clear();
