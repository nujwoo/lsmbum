...
public class LSMBTree extends AbstractLSMIndex implements ITreeIndex {
    ...
    public LSMBTree(IIOManager ioManager, List<IVirtualBufferCache> virtualBufferCaches,
            ...
            ILSMIOOperationCallbackFactory ioOpCallbackFactory, ILSMPageWriteCallbackFactory pageWriteCallbackFactory,
            boolean needKeyDupCheck, boolean hasBloomFilter, int[] btreeFields, int[] filterFields, boolean durable,
            boolean updateAware, ITracer tracer, UpdateMemo<Integer> globalUM) throws HyracksDataException {
        super(ioManager, virtualBufferCaches, diskBufferCache, fileManager, bloomFilterFalsePositiveRate, mergePolicy,
                opTracker, ioScheduler, ioOpCallbackFactory, pageWriteCallbackFactory, componentFactory,
                bulkLoadComponentFactory, filterFrameFactory, filterManager, filterFields, durable, filterHelper,
                btreeFields, tracer, globalUM);
        this.insertLeafFrameFactory = insertLeafFrameFactory;
        this.deleteLeafFrameFactory = deleteLeafFrameFactory;
        this.cmpFactories = cmpFactories;
        this.updateAware = updateAware;
...
//            	LOGGER.info(ctx.getOperation());
            	...
            	globalUM.insertOrUpdate(
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
//            			new String(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).array(), "UTF-8"),
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
            	
//            	LOGGER.info("UM SIZE: " + globalUM.getSize());
//            	LOGGER.info(globalUM.print());
            	if(ctx.getOperation() == IndexOperation.UPDATE) {
            		if(UpdateMemoConfig.CLEAN_UPON_UPDATE) {
            			if(UpdateMemoConfig.VACUUM_CLEAN) {
            				ctx.getCurrentMutableBTreeAccessor().secondaryInsertCleanVacuumClean(indexTuple, globalUM, AntimatterAwareTupleAcceptor.INSTANCE);
            			}
            			else {
            				ctx.getCurrentMutableBTreeAccessor().secondaryInsertClean(indexTuple, globalUM, AntimatterAwareTupleAcceptor.INSTANCE);
            			}
            		}
            		else {
            			if(UpdateMemoConfig.VACUUM_CLEAN) {
            				ctx.getCurrentMutableBTreeAccessor().secondaryInsertVacuumClean(indexTuple, globalUM, AntimatterAwareTupleAcceptor.INSTANCE);
            			}
            			else {
            				ctx.getCurrentMutableBTreeAccessor().secondaryInsert(indexTuple, AntimatterAwareTupleAcceptor.INSTANCE);
//                    		insert(indexTuple, ctx);
            			}
            		}
...
//        ctx.getCurrentMutableBTreeAccessor().upsertIfConditionElseInsert(tuple, AntimatterAwareTupleAcceptor.INSTANCE);
        ctx.getCurrentMutableBTreeAccessor().secondaryInsert(tuple, AntimatterAwareTupleAcceptor.INSTANCE);
//        fff
        return true;
    }
        ...
        try {
            ...
            try {
                ...
                while (scanCursor.hasNext()) {
                    scanCursor.next();
                    if(UpdateMemoConfig.CLEAN_UPON_FLUSHING) {
                    	ITupleReference tuple = scanCursor.getTuple();
                    	byte[] tupleData = tuple.getFieldData(0);
                    	int ki = tuple.getFieldCount()-2;
                    	int ti = tuple.getFieldCount()-1;
                    	
                    	boolean isOld = globalUM.cleanEntity(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                    			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                    	
                    	if(!isOld) {
                    		componentBulkLoader.add(tuple);
                    		cnt++;
//                    		LOGGER.info("FLUSHED TUPLE");
                    	}
                    	else {
//                    		LOGGER.info("CLEANED TUPLE");
                    	}
            ...
            try {
                ...
                try {
                    ...
                    while (cursor.hasNext()) {
                        ...
                        if(UpdateMemoConfig.CLEAN_UPON_MERGING) {
